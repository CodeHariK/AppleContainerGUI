                ...
                <Dialog.Popup className="premium-card animate-slide-up bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark" style={{
                    ...
                    <div className="flex-between border-b border-border-light dark:border-border-dark" style={{ padding: '16px 20px' }}>
                        ...
                        <div className="flex-center" style={{ gap: '12px' }}>
                            <button className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-text-secondary-light dark:text-text-secondary-dark" onClick={clearLogs} title="Clear Logs">
                                <Trash2 size={16} />
                            </button>
                            <Dialog.Close className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-text-secondary-light dark:text-text-secondary-dark">
                                <X size={20} />
                            </Dialog.Close>
                        </div>
                    </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column-reverse', gap: '8px' }}>
                                {logs.map((log) => (
                                    <div key={log.id} className={`p-3 rounded-lg border-l-4 ${log.isError ? 'bg-red-500/10 border-red-500' : log.isPartial ? 'bg-yellow-500/10 border-yellow-500' : 'bg-green-500/10 border-green-500'}`}>
                                        ...
                                        <div className="text-text-primary-light dark:text-text-primary-dark whitespace-pre-wrap opacity-90" style={{
                                            fontSize: '12px',
                                            lineHeight: '1.5'
                                        }}>
